...
!!! note "Runtime hardware selection"
    When [`@init_parallel_stencil`](@ref) activates the KernelAbstractions backend, the runtime hardware symbol returned by [`current_hardware`](@ref) controls which architecture executes the call. Adjust it anytime with [`select_hardware`](@ref) to switch between CPU (`:cpu`) and GPU targets (`:gpu_cuda`, `:gpu_amd`, `:gpu_metal`, `:gpu_oneapi`). See [Interactive prototyping with runtime hardware selection](@ref interactive-prototyping-with-runtime-hardware-selection) for workflow details.
...
!!! note "Runtime hardware selection"
    When [`@init_parallel_stencil`](@ref) activates the KernelAbstractions backend, the runtime hardware symbol returned by [`current_hardware`](@ref) determines which architecture executes the kernels generated by `@parallel_indices`. Update it with [`select_hardware`](@ref) to alternate between CPU (`:cpu`) and GPU targets (`:gpu_cuda`, `:gpu_amd`, `:gpu_metal`, `:gpu_oneapi`). See [Interactive prototyping with runtime hardware selection](@ref interactive-prototyping-with-runtime-hardware-selection) for workflow details.
