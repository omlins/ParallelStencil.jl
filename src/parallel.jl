...
!!! note "Advanced optional arguments"
    ...
    - `nblocks::Tuple{Integer,Integer,Integer}`: the number of blocks to be used if the package CUDA or AMDGPU or Metal was selected with [`@init_parallel_kernel`](@ref).
    - `nthreads::Tuple{Integer,Integer,Integer}`: the number of threads to be used if the package CUDA or AMDGPU or Metal was selected with [`@init_parallel_kernel`](@ref).
...
!!! note "Advanced"
    ...
    - `backendkwargs...`: keyword arguments to be passed further to CUDA or AMDGPU or Metal (ignored for Threads and Polyester).
