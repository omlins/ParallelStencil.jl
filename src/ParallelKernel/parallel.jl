...
!!! note "Runtime hardware selection"
    When [`@init_parallel_kernel`](@ref) activates the KernelAbstractions backend, the runtime hardware symbol returned by [`current_hardware`](@ref) determines which architecture executes the kernel. Adjust it anytime with [`select_hardware`](@ref) to switch between:
    - `:cpu`: CPU execution (Threads or Polyester and KernelAbstractions CPU backend).
    - `:gpu_cuda`: CUDA.jl execution.
    - `:gpu_amd`: AMDGPU.jl execution.
    - `:gpu_metal`: Metal.jl execution.
    - `:gpu_oneapi`: oneAPI execution through KernelAbstractions.
...
!!! note "Runtime hardware selection"
    When [`@init_parallel_kernel`](@ref) activates the KernelAbstractions backend, the runtime hardware symbol returned by [`current_hardware`](@ref) determines which architecture executes the kernel. Update it with [`select_hardware`](@ref) to alternate between CPU (`:cpu`) and GPU targets (`:gpu_cuda`, `:gpu_amd`, `:gpu_metal`, `:gpu_oneapi`). See [Interactive prototyping with runtime hardware selection](@ref interactive-prototyping-with-runtime-hardware-selection) for workflow details.
...
!!! note "Runtime hardware selection"
    The synchronization targets the architecture identified by the runtime hardware symbol from [`current_hardware`](@ref). Adjust it with [`select_hardware`](@ref) when KernelAbstractions is the initialized backend to synchronize the currently selected device. See [Interactive prototyping with runtime hardware selection](@ref interactive-prototyping-with-runtime-hardware-selection) for workflow details.
