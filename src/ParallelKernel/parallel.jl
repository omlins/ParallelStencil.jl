...
function parallel_call_ad(caller::Module, kernelcall::Expr, backend_kwargs_expr::Array, async::Bool, package::Symbol, posargs, kwargs)
    ...
    ad_call               = :(ParallelStencil.ParallelKernel.AD.autodiff_deferred!($ad_mode, $f_name, Const, $(annotated_args...)))  # NOTE: it is safe to inject the return type as Const as @parallel functions must always return nothing
