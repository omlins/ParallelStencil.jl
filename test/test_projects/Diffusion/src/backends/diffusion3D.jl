...
function diffusion3D(NumberType)
# Physics
...
T   = @zeros(nx, ny, nz, eltype=NumberType);
T2  = @zeros(nx, ny, nz, eltype=NumberType);
Ci  = @zeros(nx, ny, nz, eltype=NumberType);

# Initial conditions (heat capacity and temperature with two Gaussian anomalies each)
Ci .= 1.0./( cp_min .+ Data.Array{NumberType}([5*exp(-(((ix-1)*dx-lx/1.5))^2-(((iy-1)*dy-ly/2))^2-(((iz-1)*dz-lz/1.5))^2) +
                                   5*exp(-(((ix-1)*dx-lx/3.0))^2-(((iy-1)*dy-ly/2))^2-(((iz-1)*dz-lz/1.5))^2) for ix=1:size(T,1), iy=1:size(T,2), iz=1:size(T,3)]) )
T  .= Data.Array{NumberType}([100*exp(-(((ix-1)*dx-lx/2)/2)^2-(((iy-1)*dy-ly/2)/2)^2-(((iz-1)*dz-lz/3.0)/2)^2) +
...
lam, dt, dx, dy, dz = NumberType.((lam, dt, dx, dy, dz));  # Convert to NumberType
for it = 1:nt
    @parallel diffusion3D_step!(T2, T, Ci, lam, dt, dx, dy, dz);
    T, T2 = T2, T;
end

return Data.Array{NumberType}
end
