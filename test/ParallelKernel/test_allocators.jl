            ...
            @testset "mapping to package (no celldims/celltype)" begin
                ...
                elseif $package == $PKG_KERNELABSTRACTIONS
                    ...
                    for symbol in valid_symbols
                        ...
                        @test typeof(@rand(2,3))                       == typeof(KernelAbstractions.rand(backend_handle, Float16, 2, 3))
                        @test typeof(@rand(2,3, eltype=Float64))       == typeof(KernelAbstractions.rand(backend_handle, Float64, 2, 3))
                        @test typeof(@rand(2,3, eltype=DATA_INDEX))    == typeof(KernelAbstractions.rand(backend_handle, DATA_INDEX, 2, 3))
                        @test typeof(@fill(9, 2,3))                    == typeof(KernelAbstractions.fill(backend_handle, convert(Float16, 9), 2, 3))
                        @test typeof(@fill(9, 2,3, eltype=Float64))    == typeof(KernelAbstractions.fill(backend_handle, convert(Float64, 9), 2, 3))
                        @test typeof(@fill(9, 2,3, eltype=DATA_INDEX)) == typeof(KernelAbstractions.fill(backend_handle, convert(DATA_INDEX, 9), 2, 3))
